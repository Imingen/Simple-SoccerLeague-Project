...
public class SoccerTeam implements SportsTeam, Comparable<SoccerTeam>{
	...
	//Constants for point value
	//I personaly found it weird to implement constant for draw since this 
	//is equal to "++" but better code maintainability and "future updates"
	//made me put this in as a constant. There is no constant for loss since this 
	...
	public SoccerTeam(String official, String nick) throws TeamException{			
		if(official == "" || nick == ""){
			throw new TeamException("Teamname must be filled in");
		}
		else{
	...
	public void playMatch(int goalsFor, int goalsAgainst) throws TeamException{
		...
		if(goalsFor < 0 || goalsFor > 20 || goalsAgainst < 0 || goalsAgainst > 20){
			throw new TeamException("Unrealistc number of goals");
		}
		else if(goalsFor > goalsAgainst){
			matchesWon++;
			competitionPoints += pointsForWin;
			...
			matchesDrawn++;
			competitionPoints += pointsForDraw;
